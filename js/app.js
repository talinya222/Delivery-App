(() => {
    ...
    async function showContent(e) {
        const server = "https://my-json-server.typicode.com/talinya222/delivery-app/db";
        const response = await fetch(server, {
            method: "GET"
        });
        const responseResult = await response.json();
        if (response.ok) showDatabase(responseResult); else content.innerHTML = responseResult.message;
    }
    ...
    function showDatabase(data) {
        ...
        function documentActions(e) {
            ...
            if (targetElement.classList.contains("menu__link_cart") || targetElement.closest(".menu__link_cart")) {
                if (document.querySelector(".cart-list").children.length > 0) {
                    if (document.querySelector(".cart-list__item_empty-cart")) document.querySelector(".cart-list__item_empty-cart").remove();
                    ...
                    if (null !== document.querySelector(".submenu[hidden]")) {
                        document.querySelector(".submenu").removeAttribute("hidden");
                        document.documentElement.classList.add("_active-card");
                    } else {
                        document.querySelector(".submenu").setAttribute("hidden", "hidden");
                        document.documentElement.classList.remove("_active-card");
                    }
                    let totalPrice = document.querySelector(".cart-header__total");
                    let totalPriceItems = document.getElementsByClassName("cart-list__item");
                    const init = () => {
                        let totalCost = 0;
                        [ ...totalPriceItems ].forEach((item => {
                            let priceItem = item.querySelector(".cart-list__price");
                            let price = Number(priceItem.innerHTML.replace(/[^0-9,.]/g, " "));
                            totalCost += price;
                        }));
                        totalPrice.innerHTML = `Total: ${totalCost}&nbsp;&euro;`;
                    ...
                    document.querySelector(".cards-catalog__tbs-body").classList.remove("_grid");
                    document.querySelector(".cards-catalog__tbs-body").innerHTML = "";
                    document.querySelector(".cart-list").insertAdjacentHTML("beforeend", `\n\n\t\t\t\t<li class="cart-list__item cart-list__item_empty-cart">You have to go and choose some food!</li>\n\n\t\t\t`);
                }
                e.preventDefault();
        ...
        function formQuantity() {
            document.addEventListener("click", (function(e) {
                let targetElement = e.target;
                if (targetElement.closest(".cart-list__cost") || targetElement.closest("[data-quantity-plus]") || targetElement.closest("[data-quantity-minus]")) {
                    const valueElement = targetElement.closest("[data-quantity]").querySelector("[data-quantity-value]");
                    let value = parseInt(valueElement.innerHTML);
                    let priceElement = targetElement.closest(".cart-list__cost").querySelector(".cart-list__price");
                    let totalPrice = document.querySelector(".cart-header__total");
                    let totalPriceItems = document.getElementsByClassName("cart-list__item");
                    let firstTotalPrice = totalPrice.dataset.value;
                    if (targetElement.hasAttribute("data-quantity-plus")) {
                        value++;
                        priceElement.innerHTML = "";
                        priceElement.innerHTML = `${priceElement.dataset.pricevalue * value}&nbsp;&euro;`;
                        const init = () => {
                            let totalCost = 0;
                            [ ...totalPriceItems ].forEach((item => {
                                let priceItem = item.querySelector(".cart-list__price");
                    ...
                    } else if (targetElement.hasAttribute("data-quantity-minus")) {
                        --value;
                        priceElement.innerHTML = `${Number(priceElement.innerHTML.replace(/[^0-9,.]/g, " ")) - priceElement.dataset.pricevalue}&nbsp;&euro;`;
                        totalPrice.innerHTML = `Total: ${Number(totalPrice.innerHTML.replace(/[^0-9,.]/g, " ")) - Number(priceElement.dataset.pricevalue)}&nbsp;&euro;`;
                        totalPrice.dataset.value = Number(totalPrice.innerHTML.replace(/[^0-9,.]/g, " "));
                        if (+valueElement.dataset.quantityMin) {
                            if (+valueElement.dataset.quantityMin > value) value = valueElement.dataset.quantityMin;
                        } else if (value < 1) {
                            value = 1;
                            priceElement.innerHTML = `${priceElement.dataset.pricevalue}&nbsp;&euro;`;
                            totalPrice.dataset.value = firstTotalPrice;
                            totalPrice.innerHTML = `Total: ${firstTotalPrice}&nbsp;&euro;`;
                        }
                    }
        ...
        formQuantity();
        function getProducts(productButton) {
            let productItems = document.getElementsByClassName("cart-list__item");
            let productId = [ ...productItems ].map((item => item.dataset.cartPid));
            console.log(productId);
            const request = new XMLHttpRequest;
            const url = "get_db.php";
            ...
            for (let i = 0; i < productId.length; i++) {
                const params = "id_product=" + productId[i];
                request.send(params);
            }
        }
    }
